...
def OnlineBooking(request):
    if request.method == 'POST':
        ...
        MyData.Select_Room = request.POST.get('room')
        ...
        if not MyData.Check_in or not MyData.Check_out or not MyData.Select_Room:
            return HttpResponse("Check-in / Check-out / Room missing")
...
def Allemployee(request):
    if request.method == 'POST':
        ...
        print(Serch)
        data = models.Add_Employee.objects.filter(
            Q(Employee_Id=Serch) | Q(First_Name=Serch)
        )
...
def Search(request):
    if request.method == 'POST':
        ...
        print(Serch)
        # data = models.Offline_Booking.objects.filter(First_Name=Serch) or models.Offline_Booking.objects.filter(Email=Serch)
        data = models.Offline_Booking.objects.filter(
    Q(First_Name=Serch) | Q(Email=Serch)
)
...
def Add_Room_Search(request):
    if request.method == 'POST':
        ...
        data = models.Add_Room.objects.filter(
    Q(Room_Number__icontains=Serch) |
    Q(Room_Type__icontains=Serch)
)
        return render(request, 'admin/AddRoom.html',{"data": data})
...
#         except ValueError:
#             return render(request, 'HotelApp/available_rooms.html', {
#                 'rooms': [],
#                 'check_in': '',
#                 'check_out': ''
#             })

#         booked_rooms = Online_Booking.objects.filter(
#             Check_in__lt=check_out_date,
#             Check_out__gt=check_in_date,
#             Select_Room__isnull=False
#         ).values_list('Select_Room', flat=True)

#         rooms = Add_Room.objects.exclude(
#             Room_Number__in=list(booked_rooms)
#         )
...
def check_availability(request):
    ...
    booked_rooms = []
    bookings = Online_Booking.objects.exclude(Select_Room__isnull=True)
    ...
    for b in bookings:
        ...
        if b_in < check_out and b_out > check_in:
            booked_rooms.append(b.Select_Room)

    rooms = Add_Room.objects.exclude(
        Room_Number__in=booked_rooms
    )
...
#         return render(request, 'public/available_rooms.html', {
#             'rooms': rooms,
#             'check_in': check_in,
#             'check_out': check_out
#         })
...
#     return render(request, 'public/book_room.html', {
#         'room': room
#     })
...
def all(request):
    rooms = Add_Room.objects.all()
    return render(request, 'HotelApp/all.html', {'rooms': rooms})
...
#     return render(request, 'OnlineBooking.html')
...
def OnlineBooking(request):
    if request.method == 'POST':
        ...
        if not room or not raw_check_in or not raw_check_out:
            return HttpResponseBadRequest("Invalid booking data")

        # 🔥 convert "2026/03/01 18:41" → date
        try:
            check_in = datetime.strptime(raw_check_in, "%Y/%m/%d %H:%M").date()
            check_out = datetime.strptime(raw_check_out, "%Y/%m/%d %H:%M").date()
        except ValueError:
        ...
        Online_Booking.objects.create(
            ...
            Check_in=check_in,
            Check_out=check_out,
            Select_Room=room   # 🔥 THIS blocks the room
        )

        return redirect('Home')

    # GET → open booking page
    return render(request, 'HotelApp/OnlineBooking.html', {
        'room': request.GET.get('room'),
        'check_in': request.GET.get('check_in'),
        'check_out': request.GET.get('check_out'),
    })
    ...
    #     return HttpResponseBadRequest("Booking page opened incorrectly")
